     ...
     for {
       ...
       folders.value.foreach{ f =>
         ...
         if(folder.value(2).as[String] == "FILE"){
           APIDoc.createLink(sessionKey, folder.value(16).as[String], folder.value(0).as[String], folder.value(1).as[String], folder.value(4).as[String], folder.value(3).as[String]).map{ res =>
             println(res)
           }
         }
       }
