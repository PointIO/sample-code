       ...
       folders = foldersJson.getOrElse(throw new Exception("Folders not found"))
        ...
        val folder = f.as[JsArray]
          if(folder.value(2).as[String] == "FILE") {
            for {
              checkout <- Pio.fileCheckout(sessionKey, folder.value(16).as[String], folder.value(0).as[String], folder.value(1).as[String])
              download <- Pio.fileDownload(sessionKey, folder.value(16).as[String], folder.value(0).as[String], folder.value(1).as[String])
              //edit file
              localPath = ""
              upload <- Pio.fileUpload(sessionKey, folder.value(16).as[String], folder.value(0).as[String], folder.value(1).as[String], localPath)
              checkin <- Pio.fileCheckin(sessionKey, folder.value(16).as[String], folder.value(0).as[String], folder.value(1).as[String])
            } yield {
              checkin
            }
          }
